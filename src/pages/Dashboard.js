...
const POLL_INTERVAL = 10000; // refresh stats every 10 seconds
  ...
  return (
    <Layout>
      <div className="page-header">
        <h2>Overview</h2>
        <div>
          <p>Loyd's Fitness Gym â€” Owner Dashboard</p>
          {lastUpdated && (
            <p style={{ fontSize: 11, color: "var(--success)", marginTop: 2 }}>
              â— Live â€” last updated {lastUpdated.toLocaleTimeString("en-PH")}
            </p>
          )}
        </div>
      </div>
      ...
      {pending > 0 && (
        <div style={{ background:"rgba(255,179,0,0.08)", border:"1px solid rgba(255,179,0,0.3)", borderRadius:10, padding:"14px 18px", marginBottom:20, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          <div>
            <div style={{ fontWeight:700, color:"var(--warning)", fontSize:14 }}>â³ {pending} Admin Request{pending!==1?"s":""} Pending</div>
            <div style={{ fontSize:12, color:"var(--muted)", marginTop:2 }}>New admins are waiting for your approval.</div>
          </div>
          <Link to="/admins" className="btn btn-warning btn-sm">Review Now â†’</Link>
        </div>
      )}
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="stat-value yellow">{loading?"â€”":fmtInt(stats.total_members)}</div>
          <div className="stat-sub">All registered</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Active Members</div>
          <div className="stat-value green">{loading?"â€”":fmtInt(stats.active_members)}</div>
          <div className="stat-sub">Valid subscriptions</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Total Revenue</div>
          <div className="stat-value" style={{fontSize:32}}>{loading?"â€”":"â‚±"+fmt(stats.revenue)}</div>
        ...
        <div className="stat-card" style={{ borderColor: "rgba(232,255,0,0.3)", background: "linear-gradient(135deg,#1a1a1a 0%,#1f1f0a 100%)", cursor: "pointer" }}
          ...
          <div className="stat-value" style={{color:"var(--accent)",fontSize:32}}>{loading?"â€”":"â‚±"+fmt(stats.walkin_revenue_today)}</div>
          <div className="stat-sub">{stats.walkin_count_today||0} walk-ins Â· click to view live feed â†’</div>
        </div>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
        <div className="card">
          <div className="stat-label" style={{ marginBottom:12 }}>ðŸ‘¥ Admin Accounts</div>
          ...
          <Link to="/admins" className="btn btn-ghost btn-sm" style={{ marginTop:14, width:"100%", justifyContent:"center" }}>Manage Admins â†’</Link>
        </div>
        <div className="card">
          <div className="stat-label" style={{ marginBottom:12 }}>â± Gym Activity Today</div>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <Link to="/logs" className="btn btn-ghost btn-sm" style={{ marginTop:14, width:"100%", justifyContent:"center" }}>View Activity Logs â†’</Link>
        </div>
      </div>
